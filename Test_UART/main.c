...
int16_t interface_comm_get_value ( void )
{
        uint8_t bytes[2];
        msg = sdReadTimeout( &SD2, bytes, 2, MS2ST( 10 ) );

    if ( msg == 2 )
    {

        /* Now you work only with two byte - use only two byte */
        /* Otherwise, use sdReadTimeout( sdp, buffer, bytes, time ) */
